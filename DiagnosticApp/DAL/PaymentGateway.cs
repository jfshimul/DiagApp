...
namespace DiagnosticApp.DAL
{
    public class PaymentGateway
    {
        ...
        {
            Patients aPatientSummary = new Patients();            
            
            SqlConnection connection = new SqlConnection(connectionString);
            string query = "SELECT TOP 1 * FROM vwBillingSummary WHERE billNo ='" + billNo + "' Or MobileNo='" + billNo + "'";
            ...
            {
                reader.Read();
                aPatientSummary.PatientId = Convert.ToInt32(reader["PatientId"].ToString());
                aPatientSummary.BillNo=reader["billNo"].ToString();
                aPatientSummary.BillDate = Convert.ToDateTime(reader["billDate"].ToString());
                aPatientSummary.TotalFee =Convert.ToDecimal( reader["TotalFee"].ToString());
                aPatientSummary.PaidAmount = Convert.ToDecimal(reader["PaidAmount"].ToString());
                aPatientSummary.DueAmount = Convert.ToDecimal(reader["DueAmount"].ToString());
                
                reader.Close();
            }
            connection.Close();
            return aPatientSummary;
        }
        ...
        public int SavePayment(Payments aPayments)
        {
            ...
            string query = "INSERT INTO Payments (" +
                           "PatientId                                       ,PaidAmount                                   )" +
                           "VALUES(" + aPayments.PatientId + "              ," + aPayments.PaidAmount + "                 )";
